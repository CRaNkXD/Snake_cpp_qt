...
bool snake_hit_wall(const Snake &snake, const UShort &max_x, const UShort &max_y)
{
    ...
    if (snake_vec.front().position.first == max_x ||
        snake_vec.front().position.second == max_y ||
        snake_vec.front().position.first == std::numeric_limits<UShort>::max() ||
        snake_vec.front().position.second == std::numeric_limits<UShort>::max())
    {
        return true;
    }
    return false;
...
bool snake_hit_snake(const Snake &snake)
{
    ...
    return snake_vec.begin() + snake.get_length() != std::find_if(snake_vec.begin() + 1, snake_vec.begin() + snake.get_length(),
        ...
        return snake_part.position == snake_vec.front().position;
    });
}
...
{
    SnakeVec snake_vec = snake.get_snake();
    return snake_vec.front().position == food.position;
}
